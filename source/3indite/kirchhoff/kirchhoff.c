...
u64 kirchhoff_name(void* addr)
{
	...
	{
		if(w == 0)break;
		if(w->dstchiptype == _hash_)return w->dstchip;
...
void kirchhoff(int argc, char** argv)
{
	...
	wlen = 0;
	ctxlen = 0;
	kirchhoff_add(w->srcchiptype, w->srcchip);
