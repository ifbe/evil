...
void travel_arm64_one(u8* buf, int len, int cur, int ip, int* stack, int sp)
{
	int j;
	int off,far;
	u32 code;
...
void travel_arm64(int argc, char** argv)
{
	u32 at = 0;
	u32 sz = 0;
	if(argc < 2)return;
	...
	stackcount = 0;
	travel_arm64_one(buf, ret, at, 0, stacktable, stackcount);
	printf("knowncount=%d\n",knowncount);
